...
Current implementation: typed `uint8`, `int32`, `float32`, and `float64` create/open/element and rectangular-slice I/O in memory and native filesystem, plus v2/v3 group/attribute operations. Slices batch file I/O and codec work by touched chunk but still buffer the requested result. Ten independent zarr-python 3.4.0 fixtures are read by native tests, and zarr-python reads eleven MoonBit-generated stores locally, including a multi-chunk compressed slice. The remaining dtype breadth is open; GitHub CI has not yet run.
