...
Current implementation: typed `bool`, `uint8`, `uint16`, `int16`, `int32`, `float32`, and `float64` create/open/element and rectangular-slice I/O in memory and native filesystem, plus v2/v3 group/attribute operations and safe ancestor creation. Slices batch file I/O and codec work by touched chunk but still buffer the requested result. Eighteen independent zarr-python 3.4.0 fixtures are read by native tests, and zarr-python reads nineteen MoonBit-generated stores in CI, including a multi-chunk compressed slice, boolean masks, signed 16-bit values and nested groups. The remaining dtype breadth is open.
