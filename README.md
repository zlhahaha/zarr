...
Native tests read fourteen committed v2/v3 sample stores generated by zarr-python 3.4.0, covering numeric types, gzip/zlib/zstd, and nested groups. The generators are [scripts/generate_interop_fixtures.py](scripts/generate_interop_fixtures.py), [scripts/generate_compressed_fixtures.py](scripts/generate_compressed_fixtures.py), [scripts/generate_zstd_fixtures.py](scripts/generate_zstd_fixtures.py), [scripts/generate_u16_fixtures.py](scripts/generate_u16_fixtures.py), and [scripts/generate_group_fixtures.py](scripts/generate_group_fixtures.py). In the reverse direction, `moon run --target native cmd/interop` writes fifteen stores—including a multi-chunk compressed slice and v2/v3 nested groups—to the ignored `integration/.roundtrip/` directory, and [scripts/verify_moonbit_roundtrip.py](scripts/verify_moonbit_roundtrip.py) opens them with zarr-python. CI installs the pinned Python test oracle and runs both directions. The generators refuse to overwrite existing stores; remove only their generated directories before re-running locally.
