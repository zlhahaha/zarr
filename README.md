...
Native tests read twenty-five committed v2/v3 sample stores generated by zarr-python 3.4.0, covering numeric and boolean types, full-range 64-bit integers, non-finite floating fills, gzip/zlib/zstd, nested groups and v2 consolidated metadata. The generators are [scripts/generate_interop_fixtures.py](scripts/generate_interop_fixtures.py), [scripts/generate_compressed_fixtures.py](scripts/generate_compressed_fixtures.py), [scripts/generate_zstd_fixtures.py](scripts/generate_zstd_fixtures.py), [scripts/generate_u16_fixtures.py](scripts/generate_u16_fixtures.py), [scripts/generate_i16_fixtures.py](scripts/generate_i16_fixtures.py), [scripts/generate_u32_fixtures.py](scripts/generate_u32_fixtures.py), [scripts/generate_i64_fixtures.py](scripts/generate_i64_fixtures.py), [scripts/generate_group_fixtures.py](scripts/generate_group_fixtures.py), [scripts/generate_consolidated_fixture.py](scripts/generate_consolidated_fixture.py), [scripts/generate_bool_fixtures.py](scripts/generate_bool_fixtures.py), and [scripts/generate_nonfinite_fixtures.py](scripts/generate_nonfinite_fixtures.py). In the reverse direction, `moon run --target native cmd/interop`, `moon run --target native cmd/nonfinite_interop`, and `moon run --target native cmd/int64_interop` write twenty-five stores—including a multi-chunk compressed slice, boolean masks, non-finite fills, full-range signed/unsigned 64-bit integers and v2/v3 nested groups—to the ignored `integration/.roundtrip/` directory, and [scripts/verify_moonbit_roundtrip.py](scripts/verify_moonbit_roundtrip.py) opens them with zarr-python. CI installs the pinned Python test oracle and runs both directions. The generators refuse to overwrite existing stores; remove only their generated directories before re-running locally.
