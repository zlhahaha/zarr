...
- Last verified public feature CI: commit `22b48ae`, [run 36007898871](https://github.com/zlhahaha/zarr/actions/runs/36007898871), four matrix jobs successful and `zarr-0.1.0-source-package` artifact present. Recheck newer CI before claiming it is current.
...
- Ran `moon info`, `moon fmt`, `moon fmt --check`, native and wasm-gc `moon check --deny-warn` and builds. Native tests: 74/74; wasm-gc tests: 42/42. No `.mbti` interface diff. GitHub CI for this change still needs verification after push.
...
1. **P0 — Common Blosc compatibility:** assess Blosc write support, especially the v2 default path and v3 BloscCodec; keep unsupported compressors explicit. Read-only bitshuffle is complete locally, with remote CI verification pending.
